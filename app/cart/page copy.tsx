...
const Cart = () => {
  ...
  const user = useAppSelector((state) => state.account.user); // Ambil user_id jika login
  ...
  const proceedCheckout = async (asGuest: boolean) => {
    ...
    setLoading(true);
    try {
      const userId: string | number | null = asGuest ? null : user?.id ?? null;

      // Kirim semua produk dalam cart sekaligus ke API dalam satu request
      const productsToSend = cartItems.map((item) => ({
        product_id: Number(item.sizeId) || 0, // Pastikan nilai product_id tidak undefined
        user_id: userId, // user_id hanya dikirim jika bukan tamu (atau null)
        qty: item.quantity,
      }));
      ...
      const res = await dispatch(
        addToCart({
          asGuest,
          body: productsToSend, // Mengirim seluruh produk sekaligus
        })
      ).unwrap();

      // Setelah berhasil menambahkan item ke keranjang, ambil session dari response API dan simpan ke Redux Persist
      if (asGuest && res?.session) {
        // Menyimpan session ke Redux Persist
        dispatch(setCartMeta({ session: res.session }));
        console.log("Session dari API:", res.session);
      }

      console.log("Cart ID:", res?.cart_id);
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="w-full h-12 md:h-[56px] rounded-md bg-[#0E5A45] text-white text-[16px] md:text-[18px] font-semibold transition-colors hover:bg-[#0c4d3b] mb-2"
            >
              Masuk akun
            </button>
            ...
            <button
              ...
              className="w-full h-12 md:h-[56px] rounded-md border border-[#0E5A45] text-[#0E5A45] text-[16px] md:text-[18px] font-semibold mb-3 hover:bg-green-50"
            >
              Daftarkan diri
            </button>
            <button
              ...
              className="w-full h-12 md:h-[56px] rounded-md border border-[#0E5A45] text-[#0E5A45] text-[16px] md:text-[18px] font-semibold hover:bg-green-50"
            >
              Lanjut sebagai tamu
            </button>
