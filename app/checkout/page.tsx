...
const Checkout = () => {
  ...
  const itemsSubtotal = totalHarga || 0;
  const shippingCost = data.cost || 0;

  // Tentukan target amount
  let targetAmount = 0;

  if (voucherData?.voucher_type === "shipping") {
    targetAmount = shippingCost;
  } else if (["transaction", "product"].includes(voucherData?.voucher_type)) {
  ...
  if (voucherData?.discount_type === "percent") {
    discount = (voucherData.discount_value / 100) * targetAmount;
  } else {
    discount = voucherData?.discount_value || 0;
  }

  // Cap ke max_value kalau ada
  if (voucherData?.max_value) {
    discount = Math.min(discount, voucherData.max_value);
  }

  // Diskon tidak boleh melebihi target
  ...
  if (voucherData?.voucher_type === "shipping") {
    const newShipping = Math.max(shippingCost - discount, 0);
    totalAkhir = itemsSubtotal + newShipping;
  } else {
    const newItemsSubtotal = Math.max(itemsSubtotal - discount, 0);
    totalAkhir = newItemsSubtotal + shippingCost;
  }

  let discountDisplay = "";

  if (voucherData?.discount_type === "percent") {
    discountDisplay = `${voucherData.discount_value}%`;
  } else {
    discountDisplay = formatToIDR(voucherData?.discount_value || 0);
  }
