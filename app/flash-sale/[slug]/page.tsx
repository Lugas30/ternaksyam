...
const ProductDetail = () => {
  ...
  const galleryImages = useMemo(() => {
    if (activeSize?.images && activeSize.images.length > 0) {
      return activeSize.images.map((img: any) => ({
        original: `${imageUrl}/${img.path}`,
        thumbnail: `${imageUrl}/${img.path}`,
      }));
    }
    return [
      {
  ...
  }, [activeSize, productDetail, imageUrl]);
  ...
  const handleAddToCart = () => {
    ...
    dispatch(
      addItemToCart({
        ...
        imageId: activeSize.images?.[0]?.id || activeVariant.variant_id,
        imageUrl: `${imageUrl}/${
          activeSize.images?.[0]?.path || productDetail.gizi_path
        }`,
        quantity: 1,
