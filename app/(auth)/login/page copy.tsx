...
const Login = () => {
  ...
  const showNotification = (message: string, isError: boolean = false) => {
    ...
    if (isError) {
      setError(message); // Tampilkan juga error di area form
    } else {
      setError(""); // Hapus error di area form saat sukses
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    setError("");
    setIsLoading(true);
    showNotification("Memuat link Google login...", false);

    const API_URL = "https://ts.crx.my.id/api/auth/google";
    ...
    try {
      ...
      if (response.status !== 200) {
        // Axios menangani status non-2xx sebagai error, tapi ini untuk jaga-jaga
        throw new Error(
          `Gagal mengambil link otentikasi. Status: ${response.status}`
        );
      }

      const data = response.data; // Data sudah di-parse oleh Axios

      // Asumsi: Server mengembalikan JSON dengan field 'authUrl', 'url', atau 'redirectUrl'
      const authUrl = data.authUrl || data.url || data.redirectUrl;

      if (!authUrl || typeof authUrl !== "string") {
        throw new Error(
          "Respons API tidak valid: URL otentikasi tidak ditemukan."
        );
      }

      // Mengarahkan pengguna ke URL Google OAuth
      window.location.href = authUrl;

      // Catatan: Setelah redirect, kode di bawah ini tidak akan dijalankan
    } catch (err: any) {
      const errorMessage =
        err.message || "Gagal menghubungkan ke layanan Google.";
      showNotification(errorMessage, true);
      setIsLoading(false); // Matikan loading jika fetch gagal
    }
  };
      ...
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-xl p-8 flex justify-between">
        ...
        <div className="w-1/2">
          ...
          <button
            ...
            disabled={isLoading} // Tombol dinonaktifkan saat sedang loading
          >
            <svg
